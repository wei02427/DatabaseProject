...
CREATE VIEW order_view AS SELECT O.Order_ID, O.ID, GL.Game_ID, G.name, G.price, O.DataTime FROM `order` O, `gamelibrary` GL, `game` G WHERE O.Order_ID = GL.Order_ID AND GL.Game_ID = G.Game_ID ORDER BY O.Order_ID;
